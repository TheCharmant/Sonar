  ...
  try {
    ...
    if (userDoc.exists) {
      ...
      if (userData.status === 'inactive') {
        ...
        console.log(`Using frontend URL for deactivated account: ${frontendUrl}`);

        const redirectUrl = `${frontendUrl}/login?error=Your+account+has+been+deactivated.+Please+contact+an+administrator.&code=account_deactivated`;
        console.log(`Redirecting deactivated account to: ${redirectUrl}`);
    ...
    // Use FRONTEND_URL environment variable directly
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
    console.log(`Using frontend URL: ${frontendUrl}`);

    // Construct the redirect URL
    const redirectUrl = `${frontendUrl}/dashboard?token=${newToken}`;
